...
namespace chat_signalr_reactjs
{
    public class WebSocketHub : Hub
    {
        ...
        public void SendToAll(string nickname, string message)
        {
            //Clients.All.SendAsync("sendToAll", nickname, message);

            string messageTime = DateTime.Now.Hour.ToString() + ":" + DateTime.Now.Minute.ToString();
            Clients.Client(Context.ConnectionId).SendAsync("MessageMine", nickname, message, messageTime);
            Clients.AllExcept(Context.ConnectionId).SendAsync("MessageOther", nickname, message, messageTime);
        }
    }
}
